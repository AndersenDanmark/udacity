...
def how_many_days(month_number):
    """ the function takes a number as an input,which represents a month, and returns the number of days in that month"""

    #assert statement to raise an AssertionError when numbers are out of range

    #assert(month_number in range (1,13)==True),"Month number must be within 1 to 12"
...
#china's population as a multiple of Romania's
print(countries[0][2]/countries[2][2])

#Quiz 5, replace items inside a list
...
#Quiz 6, what is the value of agent[2] after the following code runs:
spy=[0,0,7]
agent=spy
spy[2]=agent[2]+1
[0,0,8]

#append operation, appends another element as a whole to the previous list.
p=[1,2]
q=[3,4]
p.append(q)
...
#the plus + operator, produces a new list
#the len operator, takes a list as input and output number of items inside the list. 
# note that the len operator only counts the original outside list, it won't coutn lists inside the original list.

#DRAM, 2GB is 2**30*2*8 =17 billion light switches
#1 byte=8 bits, 1 bit=1 light switch
#latency= time to retrieve a value, is 12 nanoseconds for DRAM. 
# It takes the CPU register 0.4 nanosecond to retrive a value.
# The latency for a disk hard drive is 7 milliseconds, about 0.001 second.
...
def get_next_target(page):
    """get the first link from that page, return the position of the the end of the quote"""
...
def get_all_links(page):
    ...
    while True:
        ...
        #the above line, sind get_next_target(page) returns two variables,which are url and end_quote.
        # url will be assginned to url, and end_quote will be assigned to endpos.
        if url:
            links.append(url)
            page = page[endpos:]
        else:
...
def crawl_web(seed):
    """while there are more pages tocrawl: pick a page from tocrawl
    if a page had already been crawled, skip it add that page to crawled
